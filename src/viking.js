...
attackMethod(attacker, defender) {
  let denfenderSlot = Math.floor(Math.random() * this.defender.length);
  let randomDefender = this.defender[denfenderSlot];

  let attackerSlot = Math.floor(Math.random() * this.attacker.length);
  let randomAttacker = this.vikingArmy[attackerSlot];

  let attack = randomDefender.receiveDamage(randomAttacker.strength);

  if (randomDefender.health <= 0) {
    this.defender.splice(denfenderSlot,1);
  };
  return attack;  
}
